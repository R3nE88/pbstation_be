...
async def crear_cliente(cliente: Cliente, token: str = Depends(validar_token)):
    ...
    cliente_dict = dict(cliente) #//TODO: mp se so es mejor asi o usar cliente_dict = cliente.model_dump(), investigar
...
async def agregar_adeudo(cliente_id: str, adeudo: Adeudo, token: str = Depends(validar_token)):
    try:
        ...
        await manager.broadcast(f"put-cliente:{cliente_id}")
...
@router.delete("/{cliente_id}/adeudos/{venta_id}", response_model=Cliente, status_code=status.HTTP_200_OK)
async def eliminar_adeudo(cliente_id: str, venta_id: str, token: str = Depends(validar_token)):
    try:
        # Verificar que el cliente existe
        cliente_existente = db_client.local.clientes.find_one({"_id": ObjectId(cliente_id)})
        if not cliente_existente:
...
async def actualizar_cliente(cliente: Cliente, token: str = Depends(validar_token)):
    ...
    await manager.broadcast(f"put-cliente:{str(ObjectId(cliente.id))}")
